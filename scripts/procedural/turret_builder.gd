...
## Procedural high-detail tank turret and main gun generator using SurfaceTool.
## Generates turret ring collar, sloped cheek armor, rear bustle/box, commander cupola,
## gun mantlet block, and barrel with multi-stage muzzle brake.
## Controls 360-degree turret rotation (yaw) and gun elevation/depression (pitch).
...
func _generate_turret_mesh() -> void:
	...
	for i in range(ring_sides):
		...
		_add_quad(st, p1_bot, p2_bot, p2_top, p1_top, Vector2(float(i)/ring_sides, 0), Vector2(float(i+1)/ring_sides, 0), Vector2(float(i+1)/ring_sides, 1), Vector2(float(i)/ring_sides, 1))

	# ---------------------------------------------------------
	# 2. MAIN TURRET BODY & WEDGE SLOPED CHEEKS
	# ---------------------------------------------------------
	var v_front_top = Vector3(half_l * 0.85, half_h * 0.7, 0.0)
	var v_front_bot = Vector3(half_l * 0.85, -half_h * 0.6, 0.0)
	var v_mantlet_port_tl = Vector3(half_l * 0.85, half_h * 0.4, -0.3)
	var v_mantlet_port_tr = Vector3(half_l * 0.85, half_h * 0.4, 0.3)
	var v_mantlet_port_bl = Vector3(half_l * 0.85, -half_h * 0.4, -0.3)
	var v_mantlet_port_br = Vector3(half_l * 0.85, -half_h * 0.4, 0.3)
	...
	# Main Roof (Roof Plate)
	_add_quad(st, v_top_bl, v_top_br, v_cheek_r, v_cheek_l, Vector2(0,0), Vector2(1,0), Vector2(1,1), Vector2(0,1))
	_add_triangle(st, v_cheek_l, v_cheek_r, v_front_top, Vector2(0,0), Vector2(1,0), Vector2(0.5,1))

	# Bottom Floor under main turret
	_add_quad(st, v_bot_cheek_l, v_bot_cheek_r, v_bot_br, v_bot_bl, Vector2(0,0), Vector2(1,0), Vector2(1,1), Vector2(0,1))
	_add_triangle(st, v_front_bot, v_bot_cheek_r, v_bot_cheek_l, Vector2(0.5,0), Vector2(1,1), Vector2(0,1))

	# Front Cheeks (Sloped Armor Wedge)
	_add_triangle(st, v_cheek_l, v_front_top, v_bot_cheek_l, Vector2(0,0), Vector2(1,0), Vector2(0,1))
	_add_triangle(st, v_front_top, v_cheek_r, v_bot_cheek_r, Vector2(0,0), Vector2(1,0), Vector2(1,1))
	_add_quad(st, v_front_top, v_front_bot, v_bot_cheek_l, v_bot_cheek_l, Vector2(0,0), Vector2(1,0), Vector2(1,1), Vector2(0,1))
	_add_quad(st, v_front_top, v_front_bot, v_bot_cheek_r, v_bot_cheek_r, Vector2(0,0), Vector2(1,0), Vector2(1,1), Vector2(0,1))

	# Main Turret Sides
	_add_quad(st, v_top_bl, v_cheek_l, v_bot_cheek_l, v_bot_bl, Vector2(0,0), Vector2(1,0), Vector2(1,1), Vector2(0,1))
	_add_quad(st, v_cheek_r, v_top_br, v_bot_br, v_bot_cheek_r, Vector2(0,0), Vector2(1,0), Vector2(1,1), Vector2(0,1))
	...
	var bustle_bot_y = -half_h * 0.2 # Angled up to clear hull deck
	...
	# Bustle roof
	_add_quad(st, b_top_bl, b_top_br, b_top_fr, b_top_fl, Vector2(0,0), Vector2(1,0), Vector2(1,1), Vector2(0,1))
	# Bustle angled floor
	_add_quad(st, b_bot_fl, b_bot_fr, b_bot_br, b_bot_bl, Vector2(0,0), Vector2(1,0), Vector2(1,1), Vector2(0,1))
	# Bustle rear plate
	_add_quad(st, b_top_br, b_top_bl, b_bot_bl, b_bot_br, Vector2(0,0), Vector2(1,0), Vector2(1,1), Vector2(0,1))
	# Bustle sides
	_add_quad(st, b_top_bl, b_top_fl, b_bot_fl, b_bot_bl, Vector2(0,0), Vector2(1,0), Vector2(1,1), Vector2(0,1))
	_add_quad(st, b_top_fr, b_top_br, b_bot_br, b_bot_fr, Vector2(0,0), Vector2(1,0), Vector2(1,1), Vector2(0,1))

	# ---------------------------------------------------------
	# 4. COMMANDER CUPOLA HATCH
	# ---------------------------------------------------------
	...
	for i in range(cupola_sides):
		...
		_add_quad(st, c1_bot, c2_bot, c2_top, c1_top, Vector2(float(i)/cupola_sides, 0), Vector2(float(i+1)/cupola_sides, 0), Vector2(float(i+1)/cupola_sides, 1), Vector2(float(i)/cupola_sides, 1))

		# Cupola roof hatch lid
		...
		_add_triangle(st, cupola_center_top, c1_top, c2_top, uv0, uv1, uv2)
...
func _generate_gun_mesh() -> void:
	...
	# ---------------------------------------------------------
	# 1. GUN MANTLET BLOCK (ARMORED PIVOT HOUSING)
	# ---------------------------------------------------------
	var m_l = 0.5
	var m_w = 0.45
	var m_h = 0.4
	...
	var m_bl = Vector3(-0.1, m_h * 1.1, -m_w * 1.1)
	var m_br = Vector3(-0.1, m_h * 1.1, m_w * 1.1)

	var mb_fl = Vector3(m_l, -m_h, -m_w)
	var mb_fr = Vector3(m_l, -m_h, m_w)
	var mb_bl = Vector3(-0.1, -m_h * 1.1, -m_w * 1.1)
	var mb_br = Vector3(-0.1, -m_h * 1.1, m_w * 1.1)

	_add_quad(st, m_fl, m_fr, mb_fr, mb_fl, Vector2(0,0), Vector2(1,0), Vector2(1,1), Vector2(0,1)) # Front shield
	_add_quad(st, m_bl, m_fl, mb_fl, mb_bl, Vector2(0,0), Vector2(1,0), Vector2(1,1), Vector2(0,1)) # Left side
	_add_quad(st, m_fr, m_br, mb_br, mb_fr, Vector2(0,0), Vector2(1,0), Vector2(1,1), Vector2(0,1)) # Right side
	_add_quad(st, m_bl, m_br, m_fr, m_fl, Vector2(0,0), Vector2(1,0), Vector2(1,1), Vector2(0,1)) # Top
	_add_quad(st, mb_fl, mb_fr, mb_br, mb_bl, Vector2(0,0), Vector2(1,0), Vector2(1,1), Vector2(0,1)) # Bottom

	# ---------------------------------------------------------
	# 2. MAIN GUN BARREL & MUZZLE BRAKE
	# ---------------------------------------------------------
	var sides = 16
	...
	for i in range(sides):
		...
		_add_quad(st, p1_base, p2_base, p2_tip, p1_tip, Vector2(u1, 0), Vector2(u2, 0), Vector2(u2, 1), Vector2(u1, 1))
		...
		_add_quad(st, s1_start, s2_start, s2_end, s1_end, Vector2(u1, 0), Vector2(u2, 0), Vector2(u2, 1), Vector2(u1, 1))
		...
		_add_quad(st, b1_start, b2_start, b2_end, b1_end, Vector2(u1, 0), Vector2(u2, 0), Vector2(u2, 1), Vector2(u1, 1))

		# Front muzzle cap ring
		_add_quad(st, b1_end, b2_end, r2 + Vector3(brake_end_x, 0, 0), r1 + Vector3(brake_end_x, 0, 0))
